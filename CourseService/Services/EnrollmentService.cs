...
namespace CourseService.Services
{
    public class EnrollmentService : IEnrollmentService
    {
        ...
        public EnrollmentService(IUnitOfWork unitOfWork, IMapper mapper, StudentGrpc.StudentGrpcClient grpcClient)
        {
        ...
        public async Task<PagedResponseModel<EnrollmentResponseModel>> GetEnrollmentsAsync(int? studentId, int? courseId, string? search, string? sort, int page, int size, string? fields, string? expand)
        {
            ...
            if (!string.IsNullOrEmpty(expand))
            {
                ...
                foreach (var exp in expansions)
                {
                    ...
                    {
                        // Student info will be fetched via gRPC later
                    }
        ...
        {
            // Verifying student existence via gRPC
            var grpcResponse = await _grpcClient.CheckStudentExistsAsync(new StudentExistsRequest { StudentId = model.StudentId });
            if (!grpcResponse.Exists)
            {
                return new ResponseModel<EnrollmentResponseModel> { Success = false, Message = "Student does not exist", Errors = new List<string> { "Student verification failed" } };
            }
